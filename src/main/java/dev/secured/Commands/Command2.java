...
public class Command2 extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException{
        ...
        if(args.length < 1){player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.RED+"Usage: /bwsstats [player]"));return;}
        String key1 = String.valueOf(Config.getStoredAPIKey());
        System.out.println(key1);
        if(key1.contains("empty")){player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.RED+"No API Key Stored! Use /bwssetkey [API Key]"));return;}
        UUID key = UUID.fromString(key1);
        HypixelHttpClient client = new ApacheHttpClient(key);
        HypixelAPI hypixelAPI = new HypixelAPI(client);
        try {
            PlayerReply.Player response = hypixelAPI.getPlayerByName(args[0]).get().getPlayer();
            ...
            int winstreak = response.getIntProperty("stats.Bedwars.winstreak", -1);
            double wins = response.getDoubleProperty("stats.Bedwars.wins_bedwars", -1);
            double losses = response.getDoubleProperty("stats.Bedwars.losses_bedwars", -1);
            double kills = response.getDoubleProperty("stats.Bedwars.kills_bedwars", -1);
            double deaths = response.getDoubleProperty("stats.Bedwars.deaths_bedwars", -1);
            double bedsbroken = response.getDoubleProperty("stats.Bedwars.beds_broken_bedwars", -1);
            double bedslost = response.getDoubleProperty("stats.Bedwars.beds_lost_bedwars", -1);
            double finalkills = response.getDoubleProperty("stats.Bedwars.final_kills_bedwars", -1);
            double finaldeaths = response.getDoubleProperty("stats.Bedwars.final_deaths_bedwars", -1);
            double sfinalkills = response.getDoubleProperty("stats.Bedwars.eight_one_final_kills_bedwars", -1);
            double sfinaldeaths = response.getDoubleProperty("stats.Bedwars.eight_one_final_deaths_bedwars", -1);
